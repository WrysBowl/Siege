...
class Reward36 : LevelReward {
    override fun giveReward(player: Player) {
        ...
        player.sendMessage(Utils.lore("<green>+ Corrupted Crystal \u272a\u272a x16"))

        player.sendMessage(Utils.lore(""))

        GoldExpListener.giveGold(player, 4000)
        Utils.giveItem(player, Crystal.tier(2).getUpdatedItem(false).asQuantity(16))
        Utils.giveItem(player, CorruptCrystal.tier(2).getUpdatedItem(false).asQuantity(16))
    }
}
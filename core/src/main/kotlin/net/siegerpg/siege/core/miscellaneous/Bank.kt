...
object Bank {
	...
	fun blockingGetBankLevelAmount(player : OfflinePlayer) : Pair<Short, Int>? {
		...
		if (cachedData != null) {
			if (cachedData.third.plusSeconds(cacheDuration.toLong())
							.isAfter(Instant.now())
			) {
		...
		connection!!.use {
			...
			// Cache
			cachedBankData[player.uniqueId] =
	...
	fun blockingGetBankLevelAmount(
		...
		connection!!.use {
			...
			stmt.setArray(
					1,
					connection.createArrayOf("VARCHAR", playerIDs.toTypedArray())
			             )
			val resultSet = stmt.executeQuery();
			while (resultSet.next()) {
				val uuid = UUID.fromString(resultSet.getString("uuid"))
				val data =
						Pair(resultSet.getShort("bankLvl"), resultSet.getInt("bankAmt"))
				map[uuid] = data
	...
	fun blockingSetBankLevel(player : OfflinePlayer, level : Short) {
		...
		connection!!.use {
			val stmt =
					connection.prepareStatement("UPDATE userData SET bankLvl=? WHERE uuid=?");
			stmt.setShort(1, level)
			stmt.setString(2, player.uniqueId.toString())
			stmt.executeUpdate()
			val cachedData = cachedBankData[player.uniqueId]
			cachedBankData[player.uniqueId] =
					Triple(level, cachedData?.second ?: 0, Instant.now())
		}
	}
	...
	fun blockingSetBankAmount(player : OfflinePlayer, amount : Int) {
		...
		connection!!.use {
			val stmt =
					connection.prepareStatement("UPDATE userData SET bankAmt=? WHERE uuid=?");
			stmt.setInt(1, amount)
			stmt.setString(2, player.uniqueId.toString())
			stmt.executeUpdate()
			val cachedData = cachedBankData[player.uniqueId]
			cachedBankData[player.uniqueId] =
					Triple(cachedData?.first ?: 0, amount, Instant.now())
		}
	}

...
enum class ArmorType(val slot : Int) {
	...
	companion object {
		...
		fun matchType(itemStack : ItemStack?) : ArmorType? {
			...
			return if (type.endsWith("_HELMET") || type.endsWith("_SKULL") || type.endsWith(
							"_HEAD"
			                                                                               ) || customItem != null
			) HELMET else if (type.endsWith(
							"_CHESTPLATE"
			                               ) || type == "ELYTRA"
			) CHESTPLATE else if (type.endsWith("_LEGGINGS")) LEGGINGS else if (type.endsWith(
							"_BOOTS"
			                                                                                 )
			) BOOTS else null
		}

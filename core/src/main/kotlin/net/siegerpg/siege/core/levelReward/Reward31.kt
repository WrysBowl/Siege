...
class Reward31 : LevelReward {
    override fun giveReward(player: Player) {
        ...
        player.sendMessage(Utils.lore("<green>+ Fairy Dust \u272a\u272a x16"))
        player.sendMessage(Utils.lore("<green>+ Cursed Bone \u272a\u272a x16"))

        player.sendMessage(Utils.lore(""))

        GoldExpListener.giveGold(player, 3500)
        Utils.giveItem(player, FairyDust.tier(2).getUpdatedItem(false).asQuantity(16))
        Utils.giveItem(player, CursedBone.tier(2).getUpdatedItem(false).asQuantity(16))
